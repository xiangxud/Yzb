...
export default class Waring extends Component{
    ...
    humidityWaring = (humidity)=>{
        if(!this.empty(humidity)){
            return (
                <View style={style.item}><Text>湿度</Text><Text style={style.red}>{humidity}</Text></View>
            )
        }
    }
    temperatureWaring = (temperature)=>{
        if(!this.empty(temperature)){
            return (
                <View style={style.item}><Text>温度</Text><Text style={style.red}>{temperature}</Text></View>
            )
        }
    }
    co2Waring = (co2)=>{
        if(!this.empty(co2)){
            return (
                <View style={style.item}><Text>二氧化碳</Text><Text style={style.red}>{co2}</Text></View>
            )
        }
    }
